...
# The published, processed v75 SFT dataset. ``--input-root`` overrides this with a
# local snapshot; otherwise the dataset is downloaded from the Hub on demand.
DEFAULT_HF_DATASET = "eewer/qwen3-4b-thinking-sft-v75-verification-strictpassed-cap4-processed"


def _resolve_input_root(args: argparse.Namespace) -> Path:
    """Return the directory whose ``data/*.jsonl.zst`` shards should be converted.

    If ``--input-root`` is given we use it directly; otherwise we download the HF
    dataset snapshot (``--hf-dataset``) and use its local path. The dataset stores
    shards under ``<root>/data/*.jsonl.zst``.
    """
    if args.input_root is not None:
        return args.input_root
    ...
    snapshot = snapshot_download(
        ...
        allow_patterns=["data/*.jsonl.zst", "*.json"],
    )
    print(f"downloaded {args.hf_dataset} -> {snapshot}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Local snapshot dir containing data/*.jsonl.zst. If omitted, the "
        "dataset is downloaded from the Hub (see --hf-dataset).",
    )
    parser.add_argument(
        "--hf-dataset",
        default=DEFAULT_HF_DATASET,
        help="HF dataset id to download when --input-root is not given.",
    )
    parser.add_argument("--hf-revision", default=None, help="Optional HF dataset revision/branch.")
    parser.add_argument(
        "--hf-cache-dir",
        type=Path,
        default=None,
        help="Optional local_dir for the HF snapshot (defaults to the HF cache).",
    )
...
def main() -> None:
    args = parse_args()
    input_root = _resolve_input_root(args)
