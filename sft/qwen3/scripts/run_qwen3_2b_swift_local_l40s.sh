...
# L40S is PCIe-only (no NVLink). For the 2B model, where per-step compute is
# small enough that the ZeRO-3 per-layer parameter all-gather is a real fraction
# of step time, ZeRO-2 (shard_grad_op, params resident, only grads + optimizer
# state sharded) measured ~13% faster than ZeRO-3 (~33.0k vs ~29.1k TPS) and
# peaks at only ~24.5 GiB — leaving ~20 GiB free in 46 GiB. So we default to
# ZeRO-2 here. Flip to ZeRO-3 with FSDP_SHARDING="full_shard auto_wrap" if needed.
...
RUN_NAME="${RUN_NAME:-qwen3_2b_thinking_v75_msswift_dp8_lr${LR}_s${MAX_STEPS}_local_l40s_${RUN_STAMP}}"
