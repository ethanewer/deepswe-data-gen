...
if [ "$NPROC_PER_NODE" -ne 8 ]; then
  echo "This single-node H200 8B recipe expects 8 GPUs; got NPROC_PER_NODE=$NPROC_PER_NODE" >&2
  exit 1
fi
...
# MODEL may be a local checkpoint dir or a Hub repo id (the default). Only validate
# local absolute paths; Hub ids are resolved/downloaded by swift inside the container.
