...
# v78: v54 data and masking with a schedule designed to scale past s49.
#
# This is a controlled follow-up to v54:
# - starts from base Qwen/Qwen3-4B-Thinking-2507
# - uses the exact v54 raw strict-passed Mini-SWE-aligned data view
# - uses no prefix expansion
# - preserves the v54 verification-aware assistant loss policy
# - keeps global batch size 16 so step numbers match v54 token exposure
# - uses WSD LR: warmup to 1e-6 by s25, hold through s50, decay to 1e-7 by s150

export MODEL_SIZE=4b
export CONFIG="${CONFIG:-configs/qwen3_4b_thinking_swe260616_v78_v54_raw_wsd_65k_sft_8gpu.yaml}"
...
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260616_v78_v54_raw_exactpolicy_wsd_from_base_65k_s200_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260616_v78_v54_raw_exactpolicy_wsd_from_base_65k_s200_assistant_h200_8gpu_sft}"
...
if [ "$MODEL" != "Qwen/Qwen3-4B-Thinking-2507" ]; then
  echo "v78 must start from base Qwen/Qwen3-4B-Thinking-2507; got MODEL=$MODEL" >&2
  exit 1
fi

if [ "$ASSISTANT_LOSS_TARGET" != "assistant" ]; then
  echo "v78 must train full assistant loss spans; got ASSISTANT_LOSS_TARGET=$ASSISTANT_LOSS_TARGET" >&2
  exit 1
fi
