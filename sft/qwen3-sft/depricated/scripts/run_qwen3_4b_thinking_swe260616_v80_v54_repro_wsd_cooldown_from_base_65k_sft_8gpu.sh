...
# v80: controlled v54 scaling test. Match effective v54 data/order/masking and
# warmup/peak LR, then cool down aggressively after the s49 window.

export MODEL_SIZE=4b
export CONFIG="${CONFIG:-configs/qwen3_4b_thinking_swe260616_v80_v54_repro_wsd_cooldown_65k_sft_8gpu.yaml}"
...
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260616_v80_v54_repro_wsd_cooldown_from_base_65k_s125_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260616_v80_v54_repro_wsd_cooldown_from_base_65k_s125_assistant_h200_8gpu_sft}"
...
if [ "$MODEL" != "Qwen/Qwen3-4B-Thinking-2507" ]; then
  echo "v80 must start from base Qwen/Qwen3-4B-Thinking-2507; got MODEL=$MODEL" >&2
  exit 1
fi

if [ "$ASSISTANT_LOSS_TARGET" != "assistant" ]; then
  echo "v80 must train full assistant loss spans; got ASSISTANT_LOSS_TARGET=$ASSISTANT_LOSS_TARGET" >&2
  exit 1
fi
