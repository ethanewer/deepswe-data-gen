...
#   - whole trajectories containing any Mini-SWE tool-call parser error prompt.
#
# Motivation:
#   v66 removes explicit "No tool calls found" rows but still keeps many other
#   tool-error recovery contexts from non-passing traces. If v66 still shows
#   invalid-tool or no-tool degradation, this stricter ablation tests whether
#   removing all parser-error context is worth the smaller dataset.
#
# Data view:
#   swerebench-v5-compaction-mixed50-cleanpatch-alltoolerr-ramped-miniswe-aligned
...
export TRAIN_RAW_ROOT="${TRAIN_RAW_ROOT:-/wbl-fast/usrs/ee/code-swe-data/data/new-synthetic-data/260617/swerebench-v5-compaction-mixed50-cleanpatch-alltoolerr-ramped-miniswe-aligned/data}"
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260617_v67_v5_mixed50_cleanpatch_alltoolerr_ramped_process_from_base_65k_lr1e6_s800_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260617_v67_v5_mixed50_cleanpatch_alltoolerr_ramped_process_from_base_65k_lr1e6_s800_assistant_h200_8gpu_sft}"
...
export VAL_EVERY_STEPS="${VAL_EVERY_STEPS:-800}"
...
export DATASET_SEED="${DATASET_SEED:-61767}"
