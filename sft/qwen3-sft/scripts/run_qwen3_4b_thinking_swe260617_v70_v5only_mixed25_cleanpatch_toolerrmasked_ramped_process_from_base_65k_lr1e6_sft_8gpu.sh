...
# Larger base-start recipe for the corrected 2026-06-17 v5-only compaction-aware
# mixed25 cleanpatch view. This excludes older unrepaired compaction rows and
# uses only recommended v5 prompt/first-turn repaired compactions, plus strict
# passed originals and ranked high-quality non-passing process traces.
#
# Motivation:
#   v68 tests parser-error turn masking on a mixed50 view. The corrected v5-only
#   mixed25 view uses substantially more selected non-passing process data while
#   avoiding the legacy compaction rows that can start mid-trajectory. The
#   longer 1800-step budget is intended to cover the larger data view without
#   changing checkpoint lineage.
#
# Data view:
#   swerebench-v5only-compaction-mixed25-cleanpatch-provenance-ramped-miniswe-aligned
...
export TRAIN_RAW_ROOT="${TRAIN_RAW_ROOT:-/wbl-fast/usrs/ee/code-swe-data/data/new-synthetic-data/260617/swerebench-v5only-compaction-mixed25-cleanpatch-provenance-ramped-miniswe-aligned/data}"
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260617_v70_v5only_mixed25_cleanpatch_toolerrmasked_ramped_process_from_base_65k_lr1e6_s1800_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260617_v70_v5only_mixed25_cleanpatch_toolerrmasked_ramped_process_from_base_65k_lr1e6_s1800_assistant_h200_8gpu_sft}"
...
export VAL_EVERY_STEPS="${VAL_EVERY_STEPS:-1800}"
...
export DATASET_SEED="${DATASET_SEED:-61770}"
