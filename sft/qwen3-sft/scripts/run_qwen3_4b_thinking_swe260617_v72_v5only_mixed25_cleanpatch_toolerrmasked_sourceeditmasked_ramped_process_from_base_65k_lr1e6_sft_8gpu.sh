...
# Follow-up to v70 for the corrected 2026-06-17 v5-only mixed25 view.
# This keeps the same base-start, no-prefix, ramped larger-data recipe, but
# additionally masks brittle direct source-construction targets such as
# `echo ... >> src/Foo.java` and `printf ... > src/index.ts`.
#
# Use this only if v70's later eval still shows source-file append/overwrite
# failures. It is prepared as a clean base-start ablation, not as a continuation
# from v70.
...
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260617_v72_v5only_mixed25_cleanpatch_toolerrmasked_sourceeditmasked_ramped_process_from_base_65k_lr1e6_s1800_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260617_v72_v5only_mixed25_cleanpatch_toolerrmasked_sourceeditmasked_ramped_process_from_base_65k_lr1e6_s1800_assistant_h200_8gpu_sft}"
...
export DATASET_SEED="${DATASET_SEED:-61772}"
...
export MASK_RISKY_SOURCE_EDIT_TARGETS="${MASK_RISKY_SOURCE_EDIT_TARGETS:-true}"
