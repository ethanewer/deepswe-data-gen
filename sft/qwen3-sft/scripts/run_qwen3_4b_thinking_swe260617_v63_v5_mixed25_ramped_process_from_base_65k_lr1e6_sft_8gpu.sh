...
# Base-start ramped-passrate curriculum recipe for the larger 2026-06-17 v5
# compaction-aware mixed25-style view.
#
# Data view:
#   swerebench-v5-compaction-mixed25-ramped-miniswe-aligned
#
# Selection/order policy:
#   - include recommended v5 prompt/first-turn repaired compactions
#   - skip superseded original/raw-compacted/first-turn/v4 rows using the v5
#     provenance sidecars
#   - add the highest-ranked available non-passing rows; exact 25% passrate is
#     not reachable after quality and task-cap filtering, so the materialized
#     view is about 27.7% passed
#   - sorted shards ramp from about 10% passed to about 45.4% passed
#   - passed and non-passing rows are interleaved inside each shard, so early
#     checkpoints still see verified submit targets
#
# Loss policy:
#   - keep full trajectories in context
#   - do not use prefix expansion
#   - use non-passing rows for reasoning/tool/process supervision
#   - mask non-passing final submit targets to avoid cloning failed terminal
#     patches
...
export TRAIN_RAW_ROOT="${TRAIN_RAW_ROOT:-/wbl-fast/usrs/ee/code-swe-data/data/new-synthetic-data/260617/swerebench-v5-compaction-mixed25-ramped-miniswe-aligned/data}"
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260617_v63_v5_mixed25_ramped_process_from_base_65k_lr1e6_s2200_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260617_v63_v5_mixed25_ramped_process_from_base_65k_lr1e6_s2200_assistant_h200_8gpu_sft}"
...
export DATASET_SEED="${DATASET_SEED:-61763}"
