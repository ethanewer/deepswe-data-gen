...
# Gentle full-data continuation from the strongest checkpoint so far
# (v27 step-50). v27 degraded when the same stream continued at 1.5e-6, so this
# keeps the 131k/high-throughput shape but lowers LR and changes the dataset
# seed to avoid replaying the same first packed examples after restarting.
...
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-checkpoints/qwen3_4b_thinking_swe260612_v30_s50_full_continue_131k_lr3e7_s200_assistant_h200_8gpu_sft/}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260612_v30_s50_full_continue_131k_lr3e7_s200_assistant_h200_8gpu_sft}"

export PACK_SIZE="${PACK_SIZE:-131072}"
...
export LR="${LR:-3.0e-7}"
export MIN_LR="${MIN_LR:-3.0e-8}"
export LR_WARMUP_STEPS="${LR_WARMUP_STEPS:-10}"
export DATASET_SEED="${DATASET_SEED:-44444}"
