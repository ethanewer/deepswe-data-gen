...
# Base-start pass-last curriculum recipe for the larger mixed25-style raw2030 view.
#
# Data view:
#   swerebench-raw2030-targeted-limitations-mixed25-passlast-miniswe-aligned
#
# Selection/order policy:
#   - all strict passed rows from raw2030
#   - the highest-ranked available non-passing rows from the mixed25 selector;
#     exact 25% passrate is not reachable with the clean no-huge/task-cap
#     candidate pool, so the materialized view is about 26% passed
...
#   - mask non-passing final submit targets to avoid directly cloning failed
#     terminal patches
...
export TRAIN_RAW_ROOT="${TRAIN_RAW_ROOT:-/wbl-fast/usrs/ee/code-swe-data/data/new-synthetic-data/260616/swerebench-raw2030-targeted-limitations-mixed25-passlast-miniswe-aligned/data}"
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260616_v59_raw2030_mixed25_passlast_process_from_base_65k_lr1e6_s1400_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260616_v59_raw2030_mixed25_passlast_process_from_base_65k_lr1e6_s1400_assistant_h200_8gpu_sft}"
...
export DATASET_SEED="${DATASET_SEED:-61659}"
