...
# mixed50 cleanpatch view, with parser-error recovery turns masked instead of
# dropping whole trajectories that contain a Mini-SWE tool-call error prompt.
#
# Motivation:
#   v67 removes every row containing a tool-call parser-error prompt, which
#   drops 5,543 otherwise usable trajectories. This recipe keeps that data but
#   masks the assistant turn immediately after a parser-error prompt, avoiding
#   direct supervision on recovery from malformed tool-call output.
#
# Data view:
...
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260617_v68_v5_mixed50_cleanpatch_toolerrmasked_ramped_process_from_base_65k_lr1e6_s1200_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260617_v68_v5_mixed50_cleanpatch_toolerrmasked_ramped_process_from_base_65k_lr1e6_s1200_assistant_h200_8gpu_sft}"
...
export DATASET_SEED="${DATASET_SEED:-61768}"
