...
# Base-start, no-prefix curriculum phase 1.
#
# This warmup uses structurally valid non-passing traces to teach reasoning and
# Mini-SWE tool use, while masking unsafe final submit targets. It intentionally
# does not use prefix expansion and does not shuffle files or JSONL rows, so the
# phase boundary is controlled by the wrapper rather than by data-loader order.
...
export TRAIN_RAW_ROOT="${TRAIN_RAW_ROOT:-/wbl-fast/usrs/ee/code-swe-data/data/new-synthetic-data/260616/swerebench-rawplus-exact-0745-curriculum-failedpass-miniswe-aligned/000_failed_langmatched/data}"
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-/scratch/ewer/qwen3-sft-local/outputs/qwen3_4b_thinking_swe260616_v52_curriculum_failedwarmup_from_base_65k_lr1e6_s100_assistant_h200_8gpu_sft}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260616_v52_curriculum_failedwarmup_from_base_65k_lr1e6_s100_assistant_h200_8gpu_sft}"
...
export DATASET_SEED="${DATASET_SEED:-61652}"
