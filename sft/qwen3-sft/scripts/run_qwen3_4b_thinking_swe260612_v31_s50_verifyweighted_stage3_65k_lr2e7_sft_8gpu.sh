...
# Stage-3 continuation from the best 4B checkpoint so far (v28 step199).
# The data is still derived from the high-quality 260612 mini-SWE-aligned
# passed-prefix rows, but upweights the patch verification transition:
#   git diff > patch.txt -> cat patch.txt -> final submit
# v28 often prepared patch.txt and then submitted without checking whether the
# generated diff was visible/non-empty, so this run is intentionally short and
# low-LR to reinforce verification without moving far from the 4/30 checkpoint.
...
export TRAIN_RAW_ROOT="${TRAIN_RAW_ROOT:-/wbl-fast/usrs/ee/code-swe-data/data/new-synthetic-data/260612/highquality-1x-duplicate-reasoning-90pct-30k-full-miniswe-aligned-passed-prefix-verifyweighted-v31/data}"
export CHECKPOINT_DIR="${CHECKPOINT_DIR:-checkpoints/qwen3_4b_thinking_swe260612_v31_s50_verifyweighted_stage3_65k_lr2e7_s150_assistant_h200_8gpu_sft/}"
export RUN_NAME="${RUN_NAME:-qwen3_4b_thinking_swe260612_v31_s50_verifyweighted_stage3_65k_lr2e7_s150_assistant_h200_8gpu_sft}"
...
export DATASET_SEED="${DATASET_SEED:-77731}"
